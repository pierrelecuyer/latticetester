 ...
 * whose points \f$\mathbb{u}_i\f$ are defined by \f$ \mathbf{u}_i = (i \mathbf{a} \bmod m)/m \f$
 * where \f$\mathbf{a} = (a_1,a_2,\dots,a_t) \in \mathbb{Z}_m^t\f$ is the generating vector;
 * see Section 5.4 of the guide.
 *
 ...
 * \f$\gcd(a_j, m) = 1\f$ for \f$j =1, \dots,t\f$.  When this condition holds,
 * each lower-dimensional projection of \f$L_t \cap [0,1)^t\f$ contains the same number
 * of points as \f$L_t\cap [0,1)^t\f$, namely \f$ m \f$ points,
 * and similarly for the projections of \f$\Lambda_t \cap [0,m)^t\f$.
 ...
 * Under this condition, it is straightforward to construct a basis for any projection
 * of \f$\Lambda_t\f$ that contains the first coordinate, and also its m-dual basis.
 * In this class, we assume that \f$a_1 = 1\f$ and that \f$\gcd(a_j, m) = 1\f$ for all \f$j\f$.
 * We exploit these conditions when building bases for the lattice, the projections,
 * and their m-duals.
 * The functions `buildBasis` and `incDimBasis` always build and update the rescaled primal basis.
 * To build only the m-dual, use `buildDualBasis`.
 */
...
public:
    ...
    * The maximal dimension `maxDim` will be the length `t` of the vector `aa`.
    ...
    * the new m-dual basis vector is simply  @f$\mathbf{w}_d = (-a_d, 0, ..., 0, 1)@f$.
    */
    ...
    * When the first coordinate is 1 and belongs to the projection, the construction is direct,
    * just by selecting the rows and columns
...
void Rank1Lattice<Int, Real>::seta(const Int &a) {
   ...
   for (int64_t i = 1; i < this->m_maxDim; i++) {
      this->m_aa[i] = (a * this->m_aa[i - 1]) % this->m_modulo;
   }
}
