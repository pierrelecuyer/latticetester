	...
	if (d > this->m_order) {
		...
		for (i = 0; i < this->m_order; i++) {
			for (j = this->m_order; j < d; j++) {
				...
				// Calculate the components of v_{i,j}. Note that the first component of the coefficient is m_aCoeff[0] here
				for (k = 0; k < this->m_order; k++)
					this->m_aux_basis[i][j] += m_aCoeff[k]
							* this->m_aux_basis[i][j - (k + 1)]
							% this->m_modulo;
			}
		}
		// Put m times the identitiy matrix to the lower right part and the zero matrix to the lower left part
...
void MRGLattice<Int, Real>::buildProjection(IntLattice<Int, Real> &projLattice,
	...
	bool case1 = true;   // What is this ???
	long d = proj.size();
	IntMat &basis = projLattice.getBasis();  // Reference to basis.
	int64_t i, j, k;
	j = 0;
	// Check if the first m_order coordinates all belong to the projection
	...
	if (proj.size() < (unsigned) this->m_order)
		case1 = false;
	// Need to construct an auxillary matrix.  Why and what it is?    Needs to be explained somewhere.
	for (i = 0; i < d; i++) {
		for (j = 0; j < d; j++) {
			if (i == j) {
				this->m_aux_basis[i][j] = 1;
	...
	if (case1) { // We first compute the first m_order row.
		...
		upperTriangularBasis(m_genTemp, basis, this->m_modulo, proj.size(),
				proj.size());
	}
}
...
void MRGLattice<Int, Real>::buildProjectionDual(
	...
	// First compute a primal basis for the projection.
	if (case1) { // We first compute the first m_order row.
		for (i = 0; i < this->m_order; i++) {
			j = 0;
			for (auto it = proj.begin(); it != proj.end(); it++, j++) {
