...
public:
     ...
     */
    NormaPalpha(const std::int64_t m, int64_t alpha, int64_t s, NormType norm =
            L2NORM);
...
double NormaPalpha::calcBound(int64_t alpha, int64_t dim) {
    ...
    if (dim <= 1) {
        std::cout << "NormaPalpha::calcBound:  dim < 2.   Returns -1"
                << std::endl;
        return -1;
    }
    if (alpha <= 1) {
        std::cout << "NormaPalpha::calcBound:  alpha < 2.   Returns -1"
                << std::endl;
        return -1;
    }
    int64_t stat = IntFactor < int64_t > ::isPrime(m_m, 0);
    if (stat != PRIME) {
        std::cout << "NormaPalpha::calcBound:  m is not prime.   Returns -1"
                << std::endl;
        return -1;
    }
    ...
    if (Term1 <= alpha * dim / (alpha - 1)) {
        ...
        std::cout << "   m < exp(alpha*dim/(alpha - 1)) for dim = " << dim
                << std::endl;
        std::cout
                << "   Assumption required for existence of theoretical calcBound is not validated."
                << std::endl;
