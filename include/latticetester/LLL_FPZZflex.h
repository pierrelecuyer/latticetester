...
long LLL_FPZZtest(mat_ZZ& B, mat_ZZ& U, double delta = 0.99, long deep = 0,
           LLLCheckFct check = 0, long verb = 0);

/**
 * This function is similar to `LLL_FP` in NTL, but only the first `m` rows and
 * first `n` columns of the matrix `B` are considered, and a basis is built
 ...
 * vector `b`,  in `b[0],..., b[d-1]`.
 */
long LLL_FPZZflex(mat_ZZ& B, long m, long n, double *b, double delta = 0.99,
	    long deep = 0, LLLCheckFct check = 0, long verb = 0);

/**
 * This function is similar to `BKZ_FP` in NTL, with the same modification
...
static void RowTransformFinish(vec_ZZ& A, double *a, long *in_a, long n)
{
   // long n = A.length();
   long i;
...
static void LLLStatus(long max_k, double t, long m, long n, const mat_ZZ& B)
{
   cerr << "---- LLL_FP status ----\n";
   cerr << "elapsed time: ";
...
static void RR_GS(mat_ZZ& B, double **B1, double **mu, 
                  ...
                  long rr_st, long k, long m_orig, long n,
...
long ll_LLL_FP(mat_ZZ& B, mat_ZZ* U, double delta, long deep, 
           ...
           double *b, double *c,
           long m, long n, long init_k, long &quit)
{
   // long n = B.NumCols();
   ...
   while (k <= m) {
      ...
      if (swap_cnt > 200000) {
         ...
         RR_GS(B, B1, mu, b, c, buf, prec,
               rr_st, k, m_orig, n, rr_B1, rr_mu, rr_b, rr_c);
      ...
      do {
         ...
         for (j = rst-1; j >= 1; j--) {
            ...
            if (t1 > half_plus_fudge) { 
               ...
               if (U) RowTransform((*U)(k), (*U)(j), MU, n);
...
void BKZStatus(double tt, double enum_time, unsigned long NumIterations, 
               ...
               unsigned long NumNoOps, long m, long n,
               const mat_ZZ& B)
{
         ...
         long beta, long prune, LLLCheckFct check)
{
   ...
   m = ll_LLL_FP(B, U, delta, 0, check, B1, mu, b, c, m, n, 1, quit);
         ...
         if ((delta - 8*red_fudge)*c[jj] > cbar) {
            ...
            if (s > 0) {
               ...
               new_m = ll_LLL_FP(B, U, delta, 0, check, 
                                B1, mu, b, c, h, n, jj, quit);
               if (new_m != h) LogicError("BKZ_FPZZ: internal error");
               if (quit) break;
            }
            else {
               // the general case
               ...
               for (i = jj; i <= kk; i++) {
                  ...
                  if (U) RowTransform2((*U)(m+1), (*U)(i), MU, n);
               }
               ...
               if (b[jj] == 0) LogicError("BKZ_FZZP: internal error");
      
               // remove linear dependencies
   
               // cerr << "general case\n";
               new_m = ll_LLL_FP(B, U, delta, 0, 0, B1, mu, b, c, kk+1, n, jj, quit);
              
               if (new_m != kk) LogicError("BKZ_FPZZ: internal error");
            ...
            if (!clean) {
               new_m = 
                  ll_LLL_FP(B, U, delta, 0, check, B1, mu, b, c, h, n, h, quit);
               if (new_m != h) LogicError("BKZ_FPZZ: internal error");
               if (quit) break;
            }
            z++;
