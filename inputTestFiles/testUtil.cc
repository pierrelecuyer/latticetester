...
TEST_CASE("testing matrix and vector functions") {
    ...
    *b = NTL::RR(-3.1412);
    ...
    ProductDiagonal(M, 4, m); // This is indeeed an error in the code. The index needs to start with 0 not 1
    CHECK(m==84);
    ProductDiagonal(M, 3, m); // This is indeeed an error in the code. The index needs to start with 0 not 1
    CHECK(m==-42);
