...
/**
 * This module contains a slightly modified version of the `LLL_FP` module of NTL.
 ...
 * In `IntLattice`, these lengths are maintained in a `vector_double` object.
 * In this module, it is always assumed implicitly that `Real = double`.
 *
 * The LLL and BKZ functions return the square length of the shortest basis vector.
...
long BKZ_FP_lt(mat_ZZ& BB, const double delta, long beta, long prune,
         long m, long n, vec_double* sqlen) {
   if (m == 0)
      m = BB.NumRows();
   if (n == 0)
