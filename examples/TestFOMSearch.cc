...
static void findBestFOMs(const Int m, const Int a0,
        FigureOfMeritM<Int, Real> *fom, bool earlyDiscard, bool fromList,
        Int inList[], int64_t numMult, Int outList[], int64_t numBest,
        bool verbose = `false`) {
    Int a = a0;
    double merit;
    fom->setLowBound(0.0);
    ...
    int64_t posMin = 0;   // Position of the smallest FOM value in the list.
    ...
    if (verbose) {
        std::cout << "Best " << numBest
                << " multipliers `a` found, and their FOMs: ";
 ...
 */
template<typename Int, typename Real>
static void compareSearchMethods (const Int m, const Int a0, NTL::vector<int64_t> t,
        int64_t numMultShort, int64_t numMultLong, int64_t numBest1, int64_t numBest2) {
    ...
    findBestFOMs(m, a0, &fomPrimal, false, false, emptyList, numMultShort, outList, numBest2, true);

    // Early discard, with BB, or primal.
    findBestFOMs(m, a0, &fomPrimal, true, false, emptyList, numMultLong, outList, numBest2, true);

    // Early discard, two stages, with LLL only to retain numBest1 on first stage, for primal.
    fomPrimal.setBB (false);
    findBestFOMs(m, a0, &fomPrimal, true, false, emptyList, numMultLong, inList, numBest1, false);
    fomPrimal.setBB (true);
    findBestFOMs(m, a0, &fomPrimal, true, true, inList, numBest1, outList, numBest2, true);

    // Same experiment for the m-dual.
    findBestFOMs(m, a0, &fomDual, false, false, emptyList, numMultShort, outList, numBest2, true);
    findBestFOMs(m, a0, &fomDual, true, false, emptyList, numMultLong, outList, numBest2, true);
    fomDual.setBB (false);
    findBestFOMs(m, a0, &fomDual, true, false, emptyList, numMultLong, inList, numBest1, false);
    fomDual.setBB (true);
    findBestFOMs(m, a0, &fomDual, true, true, inList, numBest1, outList, numBest2, true);
}

int64_t main() {

    Int m(1048573); // Modulus
    Int a0(91); // Primitive element mod 1048573, we take its powers for values of a.
    int64_t numFirstLoop = 10000;  // Number of multipliers to examine in the first round.
    int64_t numBest1 = 50;  // When doing two rounds, we retain 50 from the first round.
    ...
    testSearch (m, a0, t, numFirstLoop, numBest1, numBest2);
    return 0;
}
